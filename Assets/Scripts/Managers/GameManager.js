...
function MoveAway () {
	...
	while(countAway < gameCovers.length)
	{
		...
		for(var i:int = 0; i < gameCovers.Length; i++)
		{
			...
			{
				gameCovers[i].transform.position = Vector2.MoveTowards(gameCovers[i].transform.position, gameCovers[i].GetComponent(GameCover).destination, Time.deltaTime * timeMultiplier * 35);
			}
		}
		yield;
	}
...
function MoveBack () {
	...
	while(countTowards < gameCovers.length)
	{
		...
		for(var i:int = 0; i < gameCovers.Length; i++)
		{
			...
			{
				gameCovers[i].transform.position = Vector2.MoveTowards(gameCovers[i].transform.position, gameCovers[i].GetComponent(GameCover).origin, Time.deltaTime * timeMultiplier * 35);
			}
		}
		yield;
	}
